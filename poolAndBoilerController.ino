 ...
 * Description    : reads the temperature sensor in D2 and D4 at every TEMPERATURE_SAMPLE_INTERVAL
 * Return         : none
 *******************************************************************************/
 ...
 * Notes:           the function allows remote control of relays 1, 3 and 4
                    since relay 2 is used by the pool boiler.
                    Relay 1 is only controllable if the pool system is off
 * Returns        : returns 0 if the command was accepted, -1 if not
 *******************************************************************************/
int triggerRelay(String command)
{

  // Relay Specific Command
  // this supports "34on", "14off", "13on", "13off" for example
  // it also supports "34on5", "1on3", "13on40" to turn on relay(s) for a number of minutes
  ...
  for (i = 1; i <= 4; i++)
  {
    ...
    // allow only control of relays 1, 3 and 4
    // since relay 2 is used by the pool boiler
    if (relayNumber == 2)
    {
      Particle.publish("ERROR", "Error: cannot remote control relay 2", 60, PRIVATE);
      return -1;
    }

    // relay 1 can only be controlled when the pool system is off
    if (relayNumber == 1)
    {
      if (state != STATE_OFF)
      {
        Particle.publish("ERROR", "Error: cannot remote control relay 1 when pool system is on", 60, PRIVATE);
        return -1;
      }
    }
